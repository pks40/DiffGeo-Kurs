...
\begin{satz}
	Seien $V_1,\dots,V_k, W_1,\dots,W_l, X$ endlich-dimensionale Vektorräume. Das Tensorprodukt $V_1 \otimes \dots \otimes V_k \otimes W_1^* \otimes \dots \otimes W_l^* \otimes X$ ist kanonisch isomorph zum Raum der multilinearen Abbildungen $V_1^* \times \dots \times V_k^* \times W_1 \times \dots \times W_l \to X$ via
	\begin{align*}
		(v_1 \otimes \dots \otimes v_k \otimes \eta_1 \otimes \dots \otimes \eta_l \otimes x) (\theta_1,\dots,\theta_k, w_1,\dots,w_l) \qquad \\ = \theta_1(v_1)\cdots\theta_k(v_k) \cdot \eta_1(w_1)\cdots\eta_l(w_l) \cdot x
	\end{align*}
	und linearer Fortsetzung. \qed
\end{satz}
\begin{satz}
	Sei $V$ ein endlich-dimensionaler Vektorraum. $V^{(r,s)}$ ist kanonisch isomorph zum Raum der multilinearen Abbildungen $\underbrace{V^* \times \dots \times V^*}_\text{$r$-mal} \times \underbrace{V \times \dots \times V}_\text{$s$-mal} \to \R$. \qed
\end{satz}
...
\begin{bsp}
	\begin{enumerate}[label=(\alph*)]
		...
		\item Die Komponenten einer linearen Abbildung $f\colon V \to V$, also von $f \in V^*\otimes V \equiv V \otimes V^*$, schreiben wir als $f^i_j$. Es gilt also $f = f^i_j e_j\otimes\theta^i$.
	\end{enumerate}
\end{bsp}
\begin{lemma}
	In Komponenten gilt:
	\begin{enumerate}[label=(\alph*)]
		\item Für Tensorprodukte: $(v\otimes w)^{ij} = v^i w^j$ für $v,w \in V$ (analog für Tensoren höheren Typs)
		...
		\item Ist $T \in V^{(r,s)}$, so erhält man die Komponenten der Kontraktion $C_{p,q}$, indem man bei den Komponenten von $T$ den $p$-ten oberen Index mit dem $q$-ten unteren Index gleichsetzt und über diesen Index summiert.
