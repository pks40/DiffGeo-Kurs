     ...
     M_a&M_b \\};
		...
		\begin{bsp}\hfill
			\begin{enumerate}
				...
				\kommP{Bündelatlas konstruieren? Als Übung?}
			\end{enumerate}
		\end{bsp}
		\begin{defn}[Rahmen]
...
\begin{lemma}[Tangentialvektoren als punktweise Derivationen] \label{lemma:tangtvek_punktw_deriv}
	...
	\begin{proof}
		...
		Wir müssen natürlich zeigen, dass $w$ von der Wahl der \glqq Fortsetzung\grqq\ $\tilde f$ unabhängig ist. D.\,h. wir müssen zeigen: Stimmen $g,h \in \sC(M)$ in einer offenen Umgebung $V$ von $p$ überein, so ist $v(g) = v(h)$. Um das zu sehen, wähle eine bump function $\psi \in \sC(M)$ für $\supp\{g - h\}$\kommB{Was meinst du hier? Der Träger von $g-h$ ist doch egal...} mit Träger in $M\setminus \{p\}$. Da dann $g-h = (g-h)\psi$ gilt, ist \[v(g-h) = v((g-h)\psi) = v(g-h) \underbrace{\psi(p)}_{=0} + \underbrace{(g-h)(p)}_{=0} v(\psi) = 0.\]
...
\kommB[inline]{Insgesamt finde ich es wichtig, Zerlegungen der Eins und bump functions gesehen zu haben, aber die Beweise hier finde ich alle sehr technisch... Findest du, dass wir das machen müssen (und nicht nur sagen können: Solche Funktionen gibt es)? Also Zerlegung der Eins muss man ja quasi so machen, aber die Funktionen so explizit zu konstruieren bzw. Existenz von Allem zu zeigen ist eben sehr aufwendig, findest du nicht?}
...
\begin{defn}
	...
	\[(\phi^*X)(p) := \DD(\phi^{-1})|_{\phi(p)}(X_{\phi(p)}) \quad \text{für} \; p \in M.\]
...
<<<<<<< HEAD
	...
	die maximale Integralkurve von $X$ durch $p$ ist.
=======
	\[U_p \ni t \mapsto \Phi(p,t) \in M\]
...
>>>>>>> b6a7fca88b6d1d058b1cb4459b23222e32077504
\end{defn}
...
\begin{satz}
	...
	\begin{proof}
		... \todo{Vollständigkeit von kompakt getragenen VF beweisen.}
	\end{proof}
\end{satz}
...
<<<<<<< HEAD
	...
	\[(\mathcal L_X T)(p) := \left.\frac{\D(\Phi_t^* T)(p)}{\D t}\right|_{t=0} = \lim_{t\to 0} \frac{(\Phi_t^* T)(p) - T(p)}{t},\]
	\kommB[inline]{Vllt möchte man hier noch irgendwas dazu sagen, dass $(\Phi_t^* T)(p)$ für Vektorfelder $T$ eine Kurve im Tangentialbündel beschreibt (und man dann eben punktweise ($\forall p \in M$) die Tangentialräume des Bündels mit den Fasern identifiziert. Ich fand das damals immer irritierend, dass auf einmal überall solche $\frac{d}{dt}$ Ableitungen von Objekten auf Mf standen...}
	wobei $\Phi$ der Fluss von $X$ ist. Anschaulich gesprochen  berechnet $\mathcal L_X T$ also, wie sehr sich das geometrische Objekt $T$ ändert, wenn wir es \glqq infinitesimal\grqq\ in Richtung von $X$ fließen lassen.
=======
	Sei $M$ eine differenzierbare Mannigfaltigkeit, $X \in \mathfrak X(M)$ und entweder $T \in \sC(M)$ oder $T \in \mathfrak X(M)$. Die \emph{Lie-Ableitung von $T$ in Richtung von $X$} ist $\mathcal L_X T \in \sC(M)$ (oder $\in \mathfrak X(M)$), definiert als
	\[\mathcal L_X T := \left.\frac{\D(\Phi_t^* T)}{\D t}\right|_{t=0} = \lim_{t\to 0} \frac{\Phi_t^* T - T}{t},\]
	wobei $\Phi$ der Fluss von $X$ ist. Anschaulich gesprochen  berechnet $\mathcal L_X T$ also, wie sehr sich das geometrische Objekt $T$ ändert, wenn wir uns \glqq infinitesimal\grqq\ in Richtung von $X$ fließen lassen.
>>>>>>> b6a7fca88b6d1d058b1cb4459b23222e32077504

	Lokal um jeden Punkt $p\in M$ ist $\Phi_t$ für genügend kleines $t$ ein Diffeomorphismus, der Pullback existiert also und die Definition ergibt Sinn. Die Glattheit von $\mathcal L_X T$ folgt, da der Fluss von $X$ und $T$ glatt sind.
\end{defn}

\begin{bem}
	Ist $\Phi$ der Fluss von $X$ und $a\in\R$, so definiert $\tilde\Phi(p,t) := \Phi(p,at)$ den Fluss $\tilde\Phi$ von $aX$, und folglich ist $\left.\frac{\D(\Phi_{at}^* T)}{\D t}\right|_{t=0} = \mathcal L_{aX} T$. Andererseits ist nach Kettenregel $\left.\frac{\D(\Phi_{at}^* T)}{\D t}\right|_{t=0} = a \left.\frac{\D(\Phi_t^* T)}{\D t}\right|_{t=0} = a \mathcal L_X T$. Also gilt $\mathcal L_{aX} T = a \mathcal L_X T$.
\end{bem}
...
\begin{lemma} \label{lemma:lie_abl_funk}
	...
	\begin{proof}
		Es ist $(\mathcal L_X f)(p) = \left.\frac{\D}{\D t}(\Phi_t^* f)(p)\right|_{t=0}  = \left.\frac{\D}{\D t}f(\Phi_t(p))\right|_{t=0} = \left.\D f\right|_p \left( \left.\frac{\D}{\D t}\Phi_t(p)\right|_{t=0} \right) = \left.\D f\right|_p (X_p) = X_p(f)$ nach Kettenregel.
	\end{proof}
\end{lemma}
