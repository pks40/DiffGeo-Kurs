	...
	\section{Grundlagen}
		...
		\begin{bsp}\hfill
			...
			\begin{enumerate}
				...
				\item $\GL(n,\R)$ ist eine $n^2$-dimensionale und $\SL(n,\R)$ eine $(n^2-1)$-dimensionale Lie-Gruppe (Die komplexen Versionen haben die doppelte Dimension). Weitere Beispiele sind die orthogonalen und unitären Gruppen $\Orth(n)$ ($n(n-1)/2$-dimensional), $\SO(n)$ ($n(n-1)/2$), $\Un(n)$ ($n^2$) und $\SU(n)$ ($n^2-1$).
		...
		\begin{lemma}
			...
			\begin{proof}
				$\gamma_g:I_g\rightarrow G$ Integralkurve in $g\in G$ $\Rightarrow \gamma^{\prime}:I_p\rightarrow G, t\mapsto l_h(\gamma_p(t))$ ist Integralkurve in $h\cdot g\;\forall h\in\gamma_p(I_p)$ (Kettenregel) $\rightarrow$ Setze $\gamma_p$ fort.
     ...
     G&H \\};
			...
			\end{center}
			\begin{proof}
				\kommB{Hängt davon ab, was wir über Integralkurven so beweisen wollen}
			\end{proof}
		\end{lemma}
		\begin{defn}
