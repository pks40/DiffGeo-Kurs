...
\begin{lemma}
	...
	\begin{proof}
		...
		Dass so eine Fortsetzung $\tilde f$ tatsächlich immer existiert, sieht man so: Wir wählen irgendeine offene Umgebung $V$ von $p$ mit $\overline{V} \subset U$ und eine bump function $\psi_V \in \sC(M)$ für $\overline{V}$ mit Träger in $U$ und definieren
		\[\tilde f := \begin{cases}
			f \cdot \psi_V & \text{auf} \; U\\
			0 & \text{auf} \; M \setminus \supp(\psi_V).
		\end{cases}\]
