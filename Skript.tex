...
\begin{lemma}
	...
	\end{proof}
\end{lemma}
Insbesondere schneiden sich verschiedene Integralkurven eines Vektorfeldes nicht (wenn sie sich schneiden, dann sind sie wegen der Eindeutigkeit lokal gleich, setzen einander also fort).

Die Integralkurven eines Vektorfeldes fasst man oft zu einer einzigen Abbildung zusammen:
\begin{defn}
...
\begin{satz}[Eigenschaften von Flüssen]
	...
	\begin{enumerate}[label=(\roman*)]
		...
		\item Ist $s \in U_p$, so ist $U_{\Phi(p,s)} = \{t - s : t \in U_p\}$.
...
Eigenschaften. Defn. Lie-Ableitung (für Funktionen und Vektorfelder). Zeigen, dass Ableitung / Kommutator.
