...
\begin{bem}
	Man kann Vektorfelder in offensichtlicher Weise mit Funktionen multiplizieren: Für $f \in \sC(M), X \in \mathfrak(X)$ definiert man $fX \in \mathfrak(X)$ durch $(fX)_p := f(p) X_p$. Mit dieser Operation wird $\mathfrak X(M)$ offensichtlich zu einem $\sC(M)$-Modul.
	\kommP{Das allgemein zu Räumen von Schnitten?}
\end{bem}
