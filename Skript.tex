		...
		\begin{satz}[Lie-Algebra]
			...
			\begin{proof}
				...
				\begin{align}
					...
					\Phi&\colon T_eG\rightarrow \vf^L(G), v\mapsto (g\mapsto \DD l_g\vert_e v)\label{def:kan_Iso}
				\end{align}
				$\Phi$ ist wohldefiniert, denn für $v\in T_eG$ und $g,h\in G$ gilt:
				\begin{align*}
					\left((l_g)_*\Phi(v)\right)_h&=\DD l_g\vert_{g^{-1}\cdot h}\Phi(v)_{g^{-1}\cdot h}\\
					...
					&=\Phi(v)_h
				\end{align*}
				Außerdem gilt $\Psi\circ\Phi=\id_{T_eG}$:
				\begin{align*}
					\Psi(\Phi(v))= (g\mapsto \DD l_g\vert_e v)_e=\DD l_e\vert_e v=v
				\end{align*}
				und die Umkehrung $\Phi(\Psi(X))=X$ wird punktweise durch Ausnutzen der Linksinvarianz klar:
				\begin{align*}
				...
				\todo{Abgeschlossenheit unter Kommutatoren}
			\end{proof}
		\end{satz}
		\begin{defn}[Lie-Algebra]
			Die \emph{Lie-Algebra} einer Lie-Gruppe ist $T_eG$ mit der folgenden Verknüpfung:
			\begin{align*}
				T_eG\times T_eG\to T_eG, (v,w)\mapsto \left[\Phi(v),\Phi(w)\right]_e
			\end{align*}
			mit dem kanonischen Isomorphismus $\Phi\colon T_eG\to \vf^L(G)$ aus \ref{def:kan_Iso}.
		\end{defn}
