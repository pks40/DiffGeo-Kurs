...
\begin{satz} \label{satz:tensorprod_multilin}
	...
	\begin{align*}
		(v_1 \otimes \dots \otimes v_r \otimes \eta_1 \otimes \dots \otimes \eta_s \otimes x) (\omega_1,\dots,\omega_r, w_1,\dots,w_s) \qquad \\ = \omega_1(v_1)\cdots\omega_r(v_r) \cdot \eta_1(w_1)\cdots\eta_s(w_s).
	\end{align*}
...
\chapter{Vektorbündel}
	...
	\section{Vektorbündel}
     ...
     M_a&M_b \\};
		...
		\end{kor}
		\kommP{Woraus folgt das? Ich denke, das ist ziemlich nichttrivial \dots oder?}
...
\begin{defn}[Tensorbündel]
	...
	\begin{align*}
		T^{(r,s)}M := \bigotimes_{i=1}^r TM \otimes \bigotimes_{j=1}^s T^*M = (TM)^{\otimes r} \otimes (T^*M)^{\otimes s}
	\end{align*}
...
Aus dem (lokalen) Koordinatenrahmen $\frac{\partial}{\partial x^i}$ (von $TM$) und dem dualen Rahmen $\D x^i$ (von $T^*M$) erhält man mit dem Tensorprodukt lokale Rahmen des Tensorbündels. Man schreibt dann die Komponenten eines Tensorfeldes $A$ vom Typ $(r,s)$ mit $r$ oberen Indizes und $s$ unteren Indizes: Es ist
...
\begin{lemma}
	Die Tensorfelder vom Typ $(r,s)$, $\Gamma(T^{(r,s)}M)$, bilden einen $\sm(M)$-Modul, und darüber hinaus bilden alle Tensorfelder mit dem \glqq faserweisen\grqq\ Tensorprodukt $(A\otimes B)_p := A_p\otimes B_p$ eine $\sm(M)$-Algebra (mit der kanonischen Identifizierung $TM\otimes T^*M\cong T^*M\otimes TM$):
	\begin{align*}
...
\begin{bem}
	Haben wir ein Tensorfeld $A \in T^{(r,s)}M$, so können wir bekanntlich punktweise $A_p \in T^{(r,s)}_pM$ als $\R$-multilineare Abbildung
...
\begin{satz}[Tensorfelder als $\sm(M)$-multilineare Abbildungen -- \glqq Fundamentallemma der Tensorrechnung\grqq]
...
\todo{Pullback, Lie Ableitung, Derivation}
