	...
	\section{Konstruktionen von Vektorbündeln}
		...
		\begin{satz}[Dualität von Schnitten]\label{satz:Dualität}
			...
			\begin{proof}
				...
				Haben wir eine $\sC(M)$-lineare Abbildung $F\colon \Gamma(E)\to\sC(M)$ gegeben, so wollen wir daraus einen Schnitt $\rho$ von $E^*$ konstruieren mit $F(\sigma) = \rho(\sigma)$ für alle $\sigma \in \Gamma(E)$, und wir wollen sehen, dass dieser Schnitt eindeutig ist. Für $x\in M$ muss dann also  gelten.
