...
\begin{bem}
	...
	und linearer Fortsetzung. Sind die $V_i$ und $W$ endlich-dimensional, dann geht das auch umgekehrt.\kommB{\glqq Umgekehrt\grqq\ ist mir
	 unklar}
...
\begin{satz}
	...
	\end{align*}
	und linearer Fortsetzung. \qed
\end{satz}

\begin{defn}[Kontraktion]
	Seien $V$ ein endlich-dimensionaler Vektorraum und $r,s \in \mathbb N$. Für $1 \le p \le r$ und $1 \le q \le s$ definieren wir die \emph{Kontraktion} $C_{p,q} \colon V^{(r,s)} \to V^{(r-1,s-1)}$ durch
	\begin{align*}
		&C_{p,q}(v_1 \otimes \dots \otimes v_r \otimes \eta_1 \otimes \dots \otimes \eta_s) \\& \qquad := \eta_q(v_p) \cdot v_1 \otimes \cdots \widehat{\otimes v_p \otimes} \cdots \otimes v_r \otimes \eta_1 \otimes \cdots \widehat{\otimes \eta_q \otimes} \cdots \otimes \eta_s
	\end{align*}
	...
	Für einfache Tensoren wird bei der Kontraktion $C_{p,q}$ also der $p$-te Vektor in die $q$-te Linearform eingesetzt.
\end{defn}
...
\begin{defn}
	...
	Die Komponenten eines Tensors $T \in V^{(r,s)}$ bzgl. dieser Basis schreiben wir dann als $T^{i_1 \dots i_r}_{j_1 \dots j_s}$, also
...
\begin{lemma}
	...
	\begin{enumerate}[label=(\alph*)]
		...
		\item Ist $T \in V^{(r,s)}$, so erhält man die Komponenten der Kontraktion $C_{p,q}$, indem man bei den Komponenten von $T$ den $p$-ten oberen Index mit dem $q$-ten unteren Index gleichsetzt und über diesen Index summiert, genannt \glqq Kontraktion der Indizes\grqq.
...
\chapter{Tensorfelder}
	\section{Tensorbündel}
		...
		\end{defn}
		Wie wir sehen werden, lassen sich die meisten Konstruktionen und Identifikationen von Tensoren auf Tensorfelder übertragen.
		\begin{lemma}
			...
			Aus dem (lokalen) Koordinatenrahmen $\frac{\partial}{\partial x^i}$ (von $\vf(M)$) und dem dualen Rahmen $dx^i$ (von $\Omega^1(M)$) erhält man mit dem Tensorprodukt lokale Rahmen der Tensorfelder (Dies gilt natürlich auch für beliebige lokale Rahmen). Man schreibt dann die Komponenten eines Tensors $A$ vom Typ $(r,s)$ mit $r$ oberen Indizes und $s$ unteren Indizes:
			\begin{align*}
		...
		\begin{lemma}[Tensorfelder als $\sm(M)$-multilineare Abbildungen]
			...
			\end{align*}
			Und insbesondere in Komponenten bezüglich Koordinatenrahmen: $(A\otimes B)^{i_1 \dots i_r k_1 \dots k_t}_{j_1 \dots j_s l_1 \dots l_u} = A^{i_1 \dots i_r}_{j_1 \dots j_s} B^{k_1 \dots k_t}_{l_1 \dots l_u}$ für $A \in \Gamma(T^{(r,s)}M)$, $T \in \Gamma(T^{(t,u)}M)$.
			
		\end{lemma}
		
		\begin{defn}[Kontraktion]
			Sei $r,s \in \mathbb N$. Für $1 \le p \le r$ und $1 \le q \le s$ definieren wir die \emph{Kontraktion} $C_{p,q} \colon \Gamma(T^{(r,s)}M) \to \Gamma(T^{(r-1,s-1)}M)$ durch
	\begin{align*}
		&C_{i,j}(X_1 \otimes \dots \otimes X_r \otimes \omega_1 \otimes \dots \otimes \omega_s) \\& \qquad := \omega_j(X_i) \cdot X_1 \otimes \cdots \widehat{\otimes X_i \otimes} \cdots \otimes X_r \otimes \omega_1 \otimes \cdots \widehat{\otimes \omega_j \otimes} \cdots \otimes \omega_s
	\end{align*}
	und $\sm(M)$-lineare Fortsetzung. Dass dabei alles glatt bleibt, sieht man in Koordinaten (wo die Kontraktion von Tensorfeldern genauso aussieht wie bei Tensoren). 

	Für einfache Tensorfelder wird bei der Kontraktion $C_{i,j}$ also das $i$-te Vektorfeld in die $j$-te $1$-Form eingesetzt (im Sinne von Lemma \ref{lemm:Dualität}).
\end{defn}
