...
\begin{satz} \label{satz:tangt_vr}
	...
	\begin{proof}\let\qed\relax
		Linearität ist einfach zu zeigen (Übung). Surjektivität und Injektivität zeigen wir für den Fall $V = \R^n$; der allgemeine Fall folgt daraus durch Wahl einer Basis.

		Für $i = 1, \dots, n$ sei $\pr^i\colon \R^n\to\R$ die Projektion auf die $i$-te Komponente, also $\pr^i(v) = v^i$. Für beliebiges $v \in \R^n$ ist
		\begin{equation} \label{eq:pf_tangt_vr}
			\left.\partial_v\right|_p(\pr^i) = \left.\frac{\D\pr^i(p + h v)}{\D h}\right|_{h=0} = \left.\frac{\D(p^i + h v^i)}{\D h}\right|_{h=0} = v^i
		\end{equation}
		für alle $i$. Falls $\left.\partial_v\right|_p = 0$ gilt, ist also $v^i = \left.\partial_v\right|_p(\pr^i) = 0(\pr^i) = 0$, also $v=0$. Das zeigt, dass der Kern der betrachteten Abbildung nur die 0 enthält, also deren Injektivität. Um die Surjektivität zu zeigen, brauchen wir einen kleinen Trick.
	\end{proof}
\end{satz}

\begin{lemma}[Hadamard-Lemma] \label{lemma:Hadamard}
	Seien $U \subset \mathbb R^n$ eine offene Menge, die sternförmig um $p$ ist, und $f \in \sC(U)$. Dann gibt es glatte Funktionen $f_i \in \sC(U), i = 1,\dots,n$, sodass \[f(x) = f(p) + \sum_{i=1}^n f_i(x) (x^i - p^i).\]
	
	\begin{proof}
		Für $x \in U$ setze $y := x - p$. Dann ist
		\begin{align*}
		f(x) - f(p) &= f(p + 1\cdot y) - f(p + 0\cdot y)\\
		&= \int_0^1 \left(\frac{\D}{\D t} f(p + ty)\right) \D t\\
		\text{(Kettenregel)} \quad &= \int_0^1 \sum_{i=1}^n (\partial_i f)(p + ty) y^i \D t\\
		&= \sum_{i=1}^n \left(\int_0^1 (\partial_i f)(p + tx - tp) \D t\right) (x^i - p^i).
		\end{align*}
		Mit \[f_i(x) := \int_0^1 (\partial_i f)(p + tx - tp) \D t\] folgt also die Behauptung (die $f_i$ sind glatt wegen Kettenregel und Sätzen über das Differenzieren unter dem Integral).
	\end{proof}
\end{lemma}

\begin{proof}[Zurück zum Beweis von Satz \ref{satz:tangt_vr}]
	Mit dem Hadamard-Lemma können wir jetzt auch zeigen, dass die Abbildung $v \mapsto \left.\partial_v\right|_p$ surjektiv ist. Seien $w \in T_p\R^n$ und $f \in \sC_p(\R^n)$ beliebig, und seien $f_i \in \sC_p(\R^n)$ zu $f$ mit dem Hadamard-Lemma gewählt, sodass also
	\[f = f(p) + \sum_{i=1}^n f_i \cdot (\pr^i - p^i)\]
	gilt. Dann ist
	\begin{align}\begin{split}
		w(f) &= w\left(f(p) + \sum_{i=1}^n f_i \cdot (\pr^i - p^i)\right)\\
		\text{(Linearität, $w(\text{const.}) = 0$)} \quad &= \sum_{i=1}^n w(f_i \cdot (\pr^i - p^i))\\
		&= \sum_{i=1}^n \Big( w(f_i) \cdot {\underbrace{(\pr^i(p) - p^i)}_{=0}} + f_i(p) \cdot w(\pr^i) \Big)\\
		&= \sum_{i=1}^n f_i(p) \cdot w(\pr^i).
	\end{split}\end{align}
	Insbesondere gilt damit auch $\left.\partial_v\right|_p (f) = \sum_{j=1}^n f_i(p) \cdot \left.\partial_v\right|_p (\pr^i) \overset{\eqref{eq:pf_tangt_vr}}{=} \sum_{j=1}^n f_i(p) \cdot v^i$ für beliebige Vektoren $v \in \R^n$. Wir können also $w(f) = \left.\partial_v\right|_p (f)$ erreichen, indem wir $v^i := w(\pr^i)$ setzen.

	Da $f$ beliebig war, gilt also $w = \left.\partial_v\right|_p$. Wir können also jedes $w \in T_p\R^n$ als Richtungsableitung in Richtung eines Vektors $v\in \R^n$ schreiben.
\end{proof}
...
\begin{kor} \label{kor:entw_tangt_Rn}
	...
	\begin{proof}
		Das ergibt sich direkt aus dem Beweis von Satz \ref{satz:tangt_vr}.
	\end{proof}
\end{kor}
