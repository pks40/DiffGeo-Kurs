...
\begin{defn}
	...
	\begin{enumerate}[label=(\alph*)]
		...
		\item Sei $\{X_i\}_{i\in I}$ eine Überdeckung von $M$. Eine \emph{Verfeinerung} dieser Überdeckung ist eine Überdeckung $\{Y_j\}_{j\in J}$, sodass jedes $Y_j$ in einem der $X_i$ enthalten ist.
	\end{enumerate}
\end{defn}
\begin{lemma} \label{lemma:lok_endl_abschluss}
	Ist $\{X_i\}_{i\in I}$ lokal endlich, so ist $\overline{\bigcup_{i\in I} X_i} = \bigcup_{i\in I} \overline{X_i}$. Außerdem ist auch $\{\overline{X_i}\}_{i\in I}$ lokal endlich.
...
\end{defn}
Die Idee des Beweises der Existenz von Zerlegungen der Eins ist, die gegebenen offenen Mengen mit kleinen Kartenumgebungen zu überdecken, deren Bilder Kugeln im $\R^n$ sind, und bump functions auf diesen Kugeln zu nutzen, un die Funktionen $\psi_i$ zu konstruieren. Um das auszuführen, brauchen wir noch einen kleinen technischen Hilfssatz:
\begin{lemma}
...
\begin{defn}[Lie-Ableitung]
	...
	Lokal um jeden Punkt $p$ ist $\Phi_t$ für genügend kleines $t$ ein Diffeomorphismus, der Pullback existiert also und die Definition ergibt Sinn.
\end{defn}
