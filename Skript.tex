...
\begin{defn}[Fluss]
	Sei $X \in \mathfrak X(M)$ ein Vektorfeld, und sei $I_p \subset \R$ zu jedem $p \in M$ das maximale Existenzintervall der Integralkurve von $X$ durch $p$. Wir setzen $U := \bigcup_{p\in M} \{p\}\times I_p \subset M \times \R$. Der \emph{Fluss des Vektorfeldes $X$} ist die Abbildung $\Phi\colon U \to M$, die so definiert ist, dass zu jedem $p \in M$ die Kurve
...
\begin{satz}[Eigenschaften von Flüssen]
	...
	\begin{enumerate}[label=(\roman*)]
		...
		\item Ist $t \in I_p$ und $s \in I_{\Phi(p,t)}$, dann ist $t+s \in I_p$ und es gilt $\Phi(\Phi(p,t),s) = \Phi(p,t+s)$.
...
\begin{defn}[Lie-Ableitung]
	...
	wobei $\Phi$ der Fluss von $X$ ist. Dabei ist $t \mapsto (\Phi_t^*T)(p) \in \mathbb R$ (oder $\in T_pM$) einfach eine Abbildung von $\mathbb R$ in einen Vektorraum, von der wir die übliche Ableitung nehmen.
...
\begin{lemma}
	...
	\begin{proof}
		...
		Für jedes $f\in \sC(M)$ und $p\in M$ gilt:
		\begingroup
		\allowdisplaybreaks
		\begin{align*}
