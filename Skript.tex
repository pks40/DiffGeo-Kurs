...
\date{}
\titlehead{\centering\includegraphics[height=.4\textheight]{logo}}
\lowertitleback{Dieses Skript wurde mit \LaTeXe\ und der \KOMAScript-Klasse \textsf{scrbook} gesetzt.}
...
\begin{satz}[Satz von der impliziten Funktion] \label{satz:impl_fkt}
	...
	\end{pmatrix} (p).\]
	Ist die linke $n\times n$-Teilmatrix invertierbar, dann lässt sich die Gleichung $F(a,b) = 0$ für $a \in \R^n, b \in \R^{m-n}$ in einer Umgebung von $p$ nach $a$ auflösen. D.\,h. es gibt offene Mengen $A \subset \R^n, B \subset \R^{m-n}$ mit $p \in A\times B \subset U$ und eine glatte Abbildung $G\colon B\to A$, sodass $F(a,b) = 0 \iff a = G(b)$ gilt.
\end{satz}
...
\epigraph{The introduction of numbers as coordinates [$\ldots$] is an act of violence [$\ldots$].}{\textsc{Hermann Weyl}\\\emph{Philosophy of Mathematics and Natural Science} (1949)}
	\section{Topologie}
		...
		\begin{defn}[Kompaktheit]
			Eine Teilmenge $K\subset X$ heißt \emph{kompakt}, wenn jede offene Überdeckung (das heißt jede Familie offener Mengen $\lbrace U_i\rbrace_{i\in I}$ mit $K\subset \bigcup_{i\in I}U_i$) eine endliche Teilüberdeckung hat (also endlich viele $i_1,\ldots,i_n\in I$ existieren, sodass schon $K\subset \bigcup_{k=1}^n U_{i_k}$ gilt).
		\end{defn}
	...
	\section{Topologische und differenzierbare Mannigfaltigkeiten}
		...
		\begin{bsp}\hfill 
			\begin{enumerate}[label=(\alph*)]
				\item Der euklidische Raum $\R^n$ (und seine offenen Teilmengen) sind $n$-dimensionale differenzierbare Mannigfaltigkeiten, der oben genannte Atlas muss dazu aber erweitert werden! Im Folgenden verwenden wir die so definierte differenzierbare Struktur auf $\R^n$.
				\item Reelle, endlich-dimensionale Vektorräume sind differenzierbare Mannigfaltigkeiten.
				\item Die obigen Beispiele topologischer Mannigfaltigkeiten gelten auch für differenzierbare Mannigfaltigkeiten, allerdings müssen auch hier Atlanten vervollständigt werden.
	...
	\section{Differenzierbare Abbildungen}
		...
		\begin{lemma} \label{lemma:karte_diffeo}
			Sei $M$ eine differenzierbare Mannigfaltigkeit und $U\subset M$ eine offene Teilmenge. Eine Abbildung $\varphi\colon U \to \varphi(U) \subset \R^n$ ist genau dann ein Diffeomorphismus, wenn $(\varphi,U)$ eine Karte von $M$ ist.
			\bewUeb
		\end{lemma}
...
\begin{defn}[Funktionskeime]
	...
	Den Raum der Funktionskeime an $M$ in $p$ ist also
	 \[\sC_p(M):= \raise0.2ex\hbox{$X$} \Big/ \lower0.2ex\hbox{$\sim$}.\]
	Den Keim zu einer Funktion $f$ schreiben wir als $[f]$. Wenn keine Missverständnisse entstehen können, schreiben wir stattdessen manchmal auch $f$, um uns Notationsaufwand zu ersparen.
\end{defn}
...
\begin{bsp}
	Ist $U\subset\mathbb R^n$ offen und $p\in U$, so bilden die partiellen Ableitungsoperatoren $\left.\partial_1\right|_p, \dots, \left.\partial_n\right|_p$ eine Basis von $T_pU$ (denn es ist ja $T_pU = T_p\mathbb R^n$).
\end{bsp}

\begin{satz}[Differential von linearen Abbildungen]
	Sei $L\colon V \to W$ eine lineare Abbildung. Bzgl. der Identifikationen $T_pV \equiv V, T_{L(p)}W \equiv W$ ist $\left.\DD L\right|_p = L$. Das heißt, dass das folgende Diagramm kommutiert:
	...
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
		Äquivalent zur obigen Aussage ist also (jetzt ohne Identifizierung) $\left.\DD L\right|_p\partial_v=\left.\partial_{L(v)}\right|_{L(p)}$.
	\begin{proof}
		Für $v\in V$ ist
		\begin{align*}
...
\begin{lemma}[Differential in Kartendarstellung] \label{lemma:differential_koord}
	...
	\[\left(\left.\DD f\right\vert_p\right)^i_j=\left(\left.\mathrm{J}(y\circ f\circ x^{-1})\right|_{x(p)}\right)^i_j = \left(\partial_j(y\circ f\circ x^{-1})^i\right) (x(p)).\]
	\bewUeb
\end{lemma}
...
\begin{defn}
	Seien $M,N$ differenzierbare Mannigfaltigkeiten, $\dim M \ge \dim N$, und sei $f \in \sC(M,N)$.
	\begin{enumerate}[label=(\alph*)]
...
\begin{satz}[Satz vom regulären Wert]
	Seien $M,N$ differenzierbare Mannigfaltigkeiten, $\dim M \ge \dim N$, und sei $q\in N$ ein regulärer Wert von $f \in \sC(M,N)$. Dann ist das Urbild $S := f^{-1}(\{q\}) \subset M$ eine eingebettete Untermannigfaltigkeit von $M$ mit Dimension $\dim S = \dim M - \dim N$. Für die Tangentialräume gilt dabei
	...
	\begin{proof}
		...
		\[\left.\mathrm{J}F\right|_{x(p)} = \begin{pmatrix}
			...
			\partial_1 F^n & \dots & \partial_n F^n & \partial_{n+1} F^n & \dots & \partial_m F^n
		\end{pmatrix}(x(p))\]
		hat Rang $n$. oBdA können wir annehmen, dass die ersten $n$ Spalten dieser Matrix linear unabhängig sind (durch Vertauschen von Spalten, d.\,h. Vertauschen von Komponenten von $x$). Das bedeutet aber, dass die linke $n\times n$-Teilmatrix invertierbar ist. Nach dem Satz von der impliziten Funktion (Satz \ref{satz:impl_fkt}) lässt sich also die Gleichung $F(a,b) = 0$ für $a\in \R^n, b \in \R^{m-n}$ in einer Umgebung von $x(p)$ nach $a$ auflösen, d.\,h. es gibt eine lokal definierte glatte Abbildung $G\colon \R^{m-n} \to \R^n$ mit $F(a,b) = 0 \iff a = G(b)$. Wir haben $S$ also lokal um $p$ als Graph der glatten Abbildung $G$ dargestellt; nach Lemma \ref{lemma:umf_lokal_graph} ist $S$ also eine eingebettete Untermannigfaltigkeit der Dimension $m-n$.
...
\epigraph{A single twig breaks, but the bundle of twigs is strong.}{\textsc{Tecumseh}}
	\section{Faserbündel}
		\begin{defn}[Faserbündel]
			Ein \emph{Faserbündel} $(P,M,\pi ,F)$ ist ein Tupel aus drei Mannigfaltigkeiten (dem \emph{Totalraum} $P$, der \emph{Basis} $M$ und der \emph{typischen Faser} $F$) zusammen mit einer \emph{Projektion} $\pi\colon P\rightarrow M$, die die folgenden Eigenschaften erfüllt:
     ...
     U& \\};
			...
			Beachte, dass aus dem obigen Diagramm bereits folgt, dass $\pi$ eine surjektive Submersion ist (da $\phi$ ein Diffeomorphismus und $\mathrm{pr}_1$ eine Submersion ist).
		\end{defn}
		\begin{nota}
			Man sagt auch \glqq Faserbündel $(P,\pi)$ über $M$\grqq\ .
		\end{nota}
		\begin{lemma}\label{lemma:Karten_aus_Trivialisierungen}
			Sei eine bijektive Abbildung $\phi\colon \pi^{-1}(U)\rightarrow U\times F$ auf einem Faserbündel $(P,\pi,M,F)$ mit den obigen Eigenschaften gegeben. Da Verkettungen von Diffeomorphismen wieder Diffeomorphismen sind, ist $\phi$ genau dann ein Diffeomorphismus (und damit eine lokale Trivialisierung), wenn für alle Karten $\varphi\colon V_y\rightarrow \varphi(V_y)\subset\R^{\dim M}$ um $y\in U\subset M$ und $\psi\colon W_v\rightarrow\psi(W_v)\subset\R^{\dim F}$ um $v\in F$ die folgende Verkettung ein Diffeomorphismus (und damit nach Lemma \ref{lemma:karte_diffeo} eine Karte von $P$) ist :
			\begin{align*}
				\left.(\varphi\times\psi)\circ\phi\right\vert_{\phi^{-1}(V_y\times W_v)}\colon\underbrace{\phi^{-1}(V_y\times W_v)}_{\subset P}\rightarrow \varphi(V_y)\times\psi(W_v)\subset\R^{\dim M +\dim F}
			\end{align*}
		\end{lemma}
	...
	\section{Vektorbündel}
		\begin{defn}[Vektorbündel]
			...
			Im Kontext von Vektorbündeln \glqq gelten\grqq\ nur noch solche Trivialisierungen, die punktweise Isomorphismen sind.
		\end{defn}
		Analog zu Kartenwechseln lassen sich auch \emph{Trivialisierungswechsel} definieren:
		\begin{defn}
			Sei $(E,M,\pi,\R^n)$ ein Vektorbündel, $U,V\subset M$ mit $U\cap V\neq\emptyset$ und Trivialisierungen ${\phi_U\colon \pi^{-1}(U)\rightarrow U\times \R^n}, {\phi_V\colon \pi^{-1}(V)\rightarrow V\times \R^n}$.
		...
		\begin{bsp}\hfill
			\begin{enumerate}[label=(\alph*)]
				...
				\item Das Tangentialbündel $TM:=\bigcup_{x\in M}T_xM$ ist ein Vektorbündel vom Rang $n:=\dim M$: mithilfe von Karten von $M$ kann man auf kanonische Weise einen Bündelatlas und Karten von $TM$ konstruieren (Übung).
				...
				\end{align*}
				Man beachte, dass für jeden Punkt in $\R^2$ eine offene Umgebung existiert, sodass die Projektion auf das Möbiusbündel eine injektive, stetige Abbildung ist. Durch Einschränkung auf das Bild und Inversion erhält man Karten (und einen Atlas).
		...
		\begin{defn}[Vektorbündelhomomorphismen]\hfill\\
			...
			Ein Tupel $(F,f)$ mit $F\colon E_a\rightarrow E_b$ und $f\colon M_a\rightarrow M_b$ heißt \emph{Vektorbündelhomomorphismus}, wenn $\pi_b\circ F=f\circ\pi_a$ gilt und $F\vert_{(E_a)_x}\colon (E_a)_x\rightarrow (E_b)_{f(x)}$ eine lineare Abbildung ist. 
			
			Es kommutiert also insbesondere das folgende Diagramm:
			\begin{center}
				\begin{tikzpicture}
     ...
     M_a&M_b \\};
		...
		\begin{bem}
			Man kann Schnitte von Vektorbündeln in offensichtlicher Weise mit Funktionen multiplizieren: Für $f \in \sC(M), \sigma \in \Gamma(E)$ definiert man $f\sigma \in \Gamma(E)$ durch $(f\sigma)(x) := f(x) \sigma(x)$. Mit dieser Operation wird $\Gamma(E)$ zu einem $\sC(M)$-Modul.
		\end{bem}

		\begin{defn}[Rahmen]
			Ein \emph{lokaler Rahmen} über $U$ ist ein Tupel lokaler Schnitte $(\sigma_1,\ldots,\sigma_n)\in\Gamma(E\vert_U)$, das für alle Punkte $x\in U$ eine Basis von $E_x$ bildet.
	...
	\section{Konstruktionen von Vektorbündeln}
		\begin{defn}
			...
			\end{align*}
			wobei $\lambda\in E_x^*$. Übergangsfunktionen sind (unter Identifikation von $\R^n$ mit Spalten- und $(\R^n)^*$ mit Zeilenvektoren) durch $\tau^{\prime}=\tau^{-1}$ gegeben, sodass $(x,\lambda)\mapsto(x,\lambda\tau^{\prime}(x))$.
		\end{defn}
		Bei dieser und den Folgenden Konstruktionen erhält man einen Atlas des Vektorbündels durch solche Karten, wie sie nach Lemma \ref{lemma:Karten_aus_Trivialisierungen} durch die Trivialisierungen mittels Verkettung mit Karten des Basisraumes gegeben sind. Man konstruiert die differenzierbare Struktur des Vektorbündels (die Vervollständigung des obigen Atlanten) also gerade so, dass die angegebenen lokalen Trivialisierungen Diffeomorphismen sind.
		...
		\begin{satz}[Dualität von Schnitten]\label{satz:Dualität}
			...
			\begin{proof}
				...
				Punktweise definiert man also $\rho:M\rightarrow E^*$ implizit durch $(\rho(x))(\sigma(x)) = (F(\sigma))(x)$. Damit dies $\rho(x)$ eindeutig definiert, zeigen wir, dass wir aus der Abbildung $F$ auch alle Werte $(\rho(x))(s)\quad \forall s\in E_x$ bestimmen können.
				
				Wir müssen also zeigen, dass sich einerseits jedes $s \in E_x$ zu einem Schnitt $\sigma \in \Gamma(E)$ fortsetzen lässt mit $\sigma(x) = s$ und dass dann andererseits $(\rho(x))(\sigma(x))$ von der Wahl dieser Fortsetzung unabhängig ist. Beides geht mit Hutfunktionen, ähnlich zum Beweis von Lemma \ref{lemma:tangtvek_punktw_deriv} (Übung).
		...
		\begin{lemma}[Schnitte von Tensorprodukt und Whitney-Summe]
			...
			\begin{align*}
				\Gamma(E_a)\times\Gamma(E_b)&\rightarrow\Gamma(E_a\oplus E_b),\\
				(\sigma_a,\sigma_b)&\mapsto \sigma_a\oplus\sigma_b,\quad (\sigma_a\oplus\sigma_b)(p):=\sigma_a(p)\oplus\sigma_b(p)\\
				\Gamma(E_a)\times\Gamma(E_b)&\rightarrow\Gamma(E_a\otimes E_b),\\
...
\begin{lemma}[Eigenschaften des Kommutators] \label{lemma:kommut}
	...
	\bewUeb
\end{lemma}
\begin{bem}\label{bem:Lie-Algebra}
...
\begin{defn}
	Sei $\phi\colon M \to N$ ein Diffeomorphismus und $X \in \mathfrak X(M)$. Der \emph{Pushforward von $X$ entlang von $\phi$} ist das Vektorfeld $\phi_*X \in \mathfrak X(N)$ definiert durch
...
\begin{defn}
	Der \emph{Pullback von $Y \in \mathfrak X(N)$ entlang von $\phi\colon M \to N$} ist $\phi^*Y := (\phi^{-1})_*Y$, also
	\[(\phi^*Y)_p := \DD(\phi^{-1})|_{\phi(p)}(Y_{\phi(p)}) \quad \text{für} \; p \in M.\]
\end{defn}
...
\begin{lemma}[Pushforward in Koordinaten]
	...
	\[(\phi_*X)^i = \left(X^j \frac{\partial (y^i \circ \phi)}{\partial x^j}\right) \circ \phi^{-1}.\]
	\bewUeb
\end{lemma}
...
\begin{satz}[Eigenschaften von Flüssen]
	...
	\begin{enumerate}[label=(\roman*)]
		...
		\item Ist $t \in I_p$ und $s \in I_{\Phi(p,t)}$, dann ist $t+s \in I_p$ und es gilt $(\Phi_s\circ\Phi_t)(p) = \Phi_{t+s}(p)$.
...
\end{defn}
\begin{bem}
	Ist $X\in\mathfrak X(M)$ vollständig, so ist $\Phi^X_t$ für jedes $t\in\R$ ein Diffeomorphismus $\Phi^X_t\colon M\to M$. Wegen $\Phi^X_0 = \id_M$ und $\Phi^X_s\circ\Phi^X_t = \Phi^X_{t+s}$ ist also
	\[\R \rightarrow\Diff(M),\quad t \mapsto \Phi^X_t \]
...
\begin{defn}[Lie-Ableitung]
	...
	wobei $\Phi = \Phi^X$ der Fluss von $X$ ist. Dabei ist $t \mapsto (\Phi_t^*T)(p) \in \mathbb R$ (oder $\in T_pM$ [sic!]) einfach eine Abbildung von $\mathbb R$ in einen Vektorraum, von der wir die übliche Ableitung nehmen.
...
\begin{lemma}
	...
	\end{proof}
\end{lemma}
Dies gibt uns eine geometrische Interpretation des Vektorfeld-Kommutators: $[X,Y] = \mathcal L_X Y$ gibt an, wie sehr sich $Y$ ändert, wenn wir es entlang von $X$ fließen lassen.
...
\epigraph{Symmetry is a vast subject, significant in art and nature. Mathematics lies at its root, and it would be hard to find a better one on which to demonstrate the working of the mathematical intellect.}{\textsc{Hermann Weyl}\\\emph{Symmetry} (1952)}
	\section{Grundlagen}
		...
		\begin{defn}[Linksinvariante Vektorfelder]
			...
			\begin{equation*}
				\left((l_g)_*X\right)_h\stackrel{\text{Def.}}{=}\DD l_g\vert_{g^{-1}\cdot h}X_{g^{-1}\cdot h}=X_h\quad\quad \forall g,h\in G
			\end{equation*}
		...
		\begin{satz}
			...
			\begin{proof}
				...
				\begin{align}
					\Psi\colon\vf^L(G)&\rightarrow T_eG, X\mapsto X_e\nonumber\\
					L\colon T_eG&\rightarrow \vf^L(G), v\mapsto L(v)\label{def:kan_Iso}\\
					L(v)\colon G&\rightarrow TG, g\mapsto \DD l_g\vert_e v\nonumber
				\end{align}
				$L$ ist wohldefiniert, denn für $v\in T_eG$ und $g,h\in G$ gilt:
				\begin{align*}
					\left((l_g)_*L(v)\right)_h&=\left.\DD l_g\right\vert_{g^{-1}\cdot h}L(v)_{g^{-1}\cdot h}\\
					&=\left.\DD l_g\right\vert_{g^{-1}\cdot h}(\DD l_{g^{-1}\cdot h}\vert_e v)\\
					\text{(Kettenregel)}\quad &=\left.\DD\left(l_g\circ l_{g^{-1}}\right)\right\vert _h\DD l_h\vert_e v\\
					&=\left.\DD l_h\right\vert_e v\\
					&=L(v)_h
				\end{align*}
				Außerdem gilt $\Psi\circ L=\id_{T_eG}$:
				\begin{align*}
					\Psi(L(v))= (L(v))_e=\DD l_e\vert_e v=v
				\end{align*}
		...
		\end{defn}
		\begin{bem}
			Die Eigenschaften des Kommutators (aus Lemma \ref{lemma:kommut}) von Vektorfeldern gelten nun auch für die Lie-Klammer auf der Lie-Algebra $\mathfrak{g}$.
			
			Per Konstruktion wird der kanonische Isomorphismus $L$ zu einem Homomorphismus von Lie-Algebren (also einer $\R$-linearen Abbildung mit $\left[.,.\right]\circ(L\times L)=L\circ\left[.,.\right]$, wobei links der Kommutator und rechts die Lie-Klammer stehen).
		\end{bem}
		\begin{bsp}\hfill
	...
	\section{Die Exponentialabbildung}
		...
		\begin{lemma}\label{lemma:Eigenschaften_exp}
			Die Exponentialabbildung erfüllt die folgenden Eigenschaften:
			\begin{enumerate}[label=(\roman*)]
				\item $\exp(0)=e$
