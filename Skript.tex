...
Wir müssen \emph{intrinsisch} über Richtungen in $M$ reden können. Dabei hilft die Beobachtung, dass man das Konzept von Richtungen dazu benutzt, zu beschreiben, wie sich Dinge bei Bewegung in diese Richtung ändern. Die Idee lautet also: Richtung = Richtungsableitung! Wir werden deshalb \glqq Richtungen\grqq\ als Ableitungsoperatoren formalisieren.

Die Idee hinter der folgenden abstrakten Formalisierung von Ableitungsoperatoren ist die folgende: Ableitungen erfüllen eine Produktregel und wirken \glqq lokal\grqq. Sei im Folgenden $M$ eine differenzierbare Mannigfaltigkeit.
...
\begin{bem}
	Ist $\gamma\colon I \to M$ eine glatte Kurve und $f\colon M \to N$ glatt, so ist $f\circ\gamma \colon I \to N$ eine Kurve in $M$. Der Geschwindigkeitsvektor von dieser Kurve zum Zeitpunkt $s$ ist $(f\circ\gamma)'(s) = \left.\DD f\right|_{\gamma(s)} (\gamma'(s))$ (Übung!).
	
	Wenn wir uns auf $M$ also vom Punkt $\gamma(s)$ aus in Richtung $v := \gamma'(s)$ bewegen, dann bewegt sich unser Bild unter $f$ auf $N$ also in Richtung $\left.\DD f\right|_p(v)$. Das Differential beschreibt also in einem sinnvollen Sinne, wie sich \glqq Richtungen\grqq\ unter Abbildungen zwischen Mannigfaltigkeiten verhalten.
\end{bem}
...
Offensichtlich gelten $\Gamma(T^{(1,0)}M)=\vf(M)$ und $\Gamma(T^{(0,0)}M)=\sm(M)$. Die Schnitte von $T^{(0,1)}M=T^*M$ heißen \emph{1-Formen} und man schreibt für diese auch $\Omega^1(M)$ (siehe Kapitel \ref{chap:Differentialformen}).
			...
			\kommP[inline]{Hier habe ich wieder zurückkorrigiert, denn Rahmen gibt es von Bündeln, nicht von Schnitten. Außerdem habe ich wieder dazugeschrieben, dass die Komponenten glatt sind.}
		\end{defn}
		...
		\begin{lemma}
			...
			\kommP{Das hab ich hier hoch geschoben von unten, weil es hier besser hinpasst.}
		...
		\begin{satz}[Tensorfelder als $\sm(M)$-multilineare Abbildungen]
			...
			\begin{align*}
				A\colon \underbrace{\Omega^1(M)\times\ldots\times\Omega^1(M)}_{r-mal}\times\underbrace{\vf(M)\times\ldots\vf(M)}_{s-mal}\rightarrow \sm(M)
			\end{align*}
		...
		\begin{defn}[Tensorderivation]
			Eine (globale) Tensorderivation ist eine Familie von $\R$-linearen Abbildungen $\delta\colon\Gamma(T^{(r,s)}M)\rightarrow \Gamma(T^{(r,s)}M)$, die die folgenden Eigenschaften erfüllt:
			\begin{enumerate}[label=\arabic*.]
