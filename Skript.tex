...
\begin{lemma}
	...
	\begin{proof}
		Sei $\dim M = m$ etc. Für $p_0 \in N$ wählen wir Karten $(\varphi,U)$ von $M$ um $p_0$ bzw. $(\psi,V)$ von $K$ um $f(p_0)$ wie oben, bzgl. derer $N$ bzw. $L$ also lokal die Form $\{p \in M \mid \varphi^{n+1}(p) = \dots = \varphi^{m}(p) = 0\}$ bzw. $\{q \in K \mid \psi^{n+1}(qp) = \dots = \psi^{m}(q) = 0\}$ haben. Dann sind also $\tilde\varphi = (\varphi^1, \dots, \varphi^n)$ eine Karte von $N$ und $\tilde\psi = (\psi^1, \dots, \psi^n)$ eine Karte von $L$.

		Sei $F := \psi \circ f|_N \circ \varphi^{-1} \colon \varphi(U) \to \psi(V)$ die Kartendarstellung von $f$ bzgl. $\varphi$ und $\psi$. Die Kartendarstellung von $f|_N$ bzgl. $\tilde\varphi$ und $\tilde\psi$ ist dann gegeben durch
